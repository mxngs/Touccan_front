...
const Avaliacao = () => {
  ...
  useEffect(() => {
    const fetchTrabalho = async () => {
      try {
        const response = await fetch(`https://touccan-backend-8a78.onrender.com/bico/historico/${id}`);
        if (!response.ok) {
  ...
  const handleEnviarAvaliacao = async () => {
    ...
    try {
      const response = await fetch('https://touccan-backend-8a78.onrender.com/avaliacao/cliente', {
        method: 'POST',
        headers: {
